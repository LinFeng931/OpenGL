...
public:
    ...
    CCube _wing[2];
    void setLightObjScale(glm::vec3);
    void setLightLevel(int level);
    void updateMotion2(float dt, glm::vec3 center); //繞點旋轉
    void updateMotion(float dt, glm::vec3 center); //抵達某點
    void setTargetCenter(glm::vec3 target); //要繞哪一個點旋轉
    void setTargetArrive(glm::vec3 target); //要抵達的點
    bool getMotionOn();
    bool getisArrive();
...
private:
    ...
    bool flag_ambient = true;
    bool flag_diffuse = true;
    bool flag_specular = true;
    bool flag_intensity = true;
    bool flag_attenuation = true;
    ...
    int _activeType; //0=normal, 1=緩慢閃爍
    int _flashingflag;
    bool _lighingOnflag;
    float _wingsRotate;
    bool _wingsRotateFlag;
    float _wingTimer;

    glm::vec3 _targetPos;    // 當前目標位置
    float _moveSpeed = 2.0f; // 每秒移動距離

    glm::vec3 _prevPos = getPos(); //旋轉前一偵的位置
    glm::vec3 _targetCenter;
    glm::vec3 _targetArrive;
