...
CShape::CShape()
{
	...
	_diffMapLoc = -1;  _lightMapLoc = -1; _normalMapLoc = -1; _cubicMapLoc = -1;
}
...
void CShape::setShaderID(GLuint shaderID, int shadeingmode)
{
	...
	glUniform1i(_diffMapLoc, 0); // diffuse map 對應 GL_TEXTURE0
	_lightMapLoc = glGetUniformLocation(_shaderProg, "uLightMap");
	glUniform1i(_lightMapLoc, 1); // light map 對應 GL_TEXTURE1
	_normalMapLoc = glGetUniformLocation(_shaderProg, "uNormalMap");
	glUniform1i(_normalMapLoc, 2); // normal map 對應 GL_TEXTURE2
	_cubicMapLoc = glGetUniformLocation(_shaderProg, "uEnvMap");
	glUniform1i(_cubicMapLoc, 3); // cubic map 對應 GL_TEXTURE3
...
{
	_rotateAngle = angle;                      // 記住角度（度）
	_rotateAxis = glm::normalize(axis);        // 記住軸向
...
void CShape::setRotate(const glm::mat4& rotMatrix)
{
	_mxRotation = rotMatrix; // 直接設整個旋轉矩陣
	_bRotation = true;

	// 如果你還有其他地方用到這些紀錄資訊，可以一起更新
	_rotateAngle = 0.0f; // 或者 -1 表示用矩陣旋轉
	_rotateAxis = glm::vec3(0.0f); // 無意義（因為不是用 angle/axis 方式）
}
